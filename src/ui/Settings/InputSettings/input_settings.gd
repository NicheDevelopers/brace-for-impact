...
@onready var input_button_scene = preload("res://src/ui/Settings/InputSettings/KeybindButton.tscn")
@onready var error_notification = $Notification
...
func _input(event: InputEvent) -> void:
	...
	if event is not InputEventKey and (event is not InputEventMouseButton or not event.pressed): return
	...
	for action in InputMap.get_actions():
		...
		if keybind == event.as_text() and action != remapped_button.find_child("ActionLabel").text:
			error_notification.show_message(event.as_text() + " is already used by action \"" + action + "\".")
